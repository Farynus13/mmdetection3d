...
def update_tier4_dataset_infos(pkl_path, out_dir):
    ...
    for i, ori_info_dict in enumerate(
        ...
        temp_data_info['lidar_points']['num_pts_feats'] = ori_info_dict.get(
            'num_features', 5)
        file_suffix_splitted = ori_info_dict['lidar_path'].split(os.sep)
        file_suffix = os.sep.join(file_suffix_splitted[-4:])
        temp_data_info['lidar_points']['lidar_path'] = file_suffix
        temp_data_info['lidar_points'][
            'lidar2ego'] = convert_quaternion_to_matrix(
                ori_info_dict['lidar2ego_rotation'],
                ori_info_dict['lidar2ego_translation'])
...
def update_pkl_infos(dataset, out_dir, pkl_path):
    ...
    elif dataset.lower() == 'tier4':
        update_tier4_dataset_infos(pkl_path=pkl_path, out_dir=out_dir)
    else:
